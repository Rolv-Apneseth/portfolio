...
pub fn NavLink(#[prop()] title: String, #[prop()] is_visible: ReadSignal<bool>) -> impl IntoView {
    view! {
        <li class=move || if is_visible.get() { "section-in-view" } else { "" }>
            <a
                class="flex relative gap-1 items-center py-1 w-max text-lg capitalize duration-300 lg:text-xl motion-safe:transition-text-color after:absolute after:bottom-0 after:left-0 after:w-0 after:bg-slate-950 after:h-[2px] after:hover:w-full after:motion-safe:transition-width after:duration-300 before:content-['->'] dark:hover:text-slate-200 dark:after:bg-slate-200
                hover:text-black before:text-violet-600 before:dark:text-purple-500 before:font-bold"
